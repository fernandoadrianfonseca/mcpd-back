...
public interface PedidoDeAdquisicionRepository extends JpaRepository<PedidoDeAdquisicion, Long> {
    ...
    @Query(value = """
        SELECT
            ...
            p.adquisicion AS adquisicion,
            p.nuevoSistema AS nuevoSistema,
            p.pañol AS pañol,
            p.entregado AS entregado
    ...
    SELECT
        ...
        AND p.pañol = 1
        AND p.hacienda = 1
        AND p.nuevoSistema = 1
        AND EXISTS (
              SELECT 1
              FROM comprasAdquisicionPedidoDetalle d
              WHERE d.comprasAdquisicionPedido = p.numero
        )
        AND NOT EXISTS (
            SELECT 1
            FROM comprasAdquisicionPedidoDetalle d
            JOIN productos_stock s ON d.productoStockId = s.id
            WHERE d.comprasAdquisicionPedido = p.numero
            AND (s.cantidad - s.cantidad_custodia) < d.cantidad
        )
        ORDER BY p.fechasolicitud DESC
        """, nativeQuery = true)
    List<Object[]> findPedidosConStockDisponible();
}
